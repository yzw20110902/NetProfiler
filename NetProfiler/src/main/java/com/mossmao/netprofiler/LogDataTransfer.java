...
public class LogDataTransfer implements DataTransfer {
    ...
    public void sendRequest(String id, Request request) throws IOException {
        ...
        if (body != null) {
            body.writeTo(buffer);
            //Log.i("-------->",buffer.readString(Charset.defaultCharset()));
            if (body.contentType().toString().contains("application/json")){
                largeLog(id, MessageType.REQUEST_BODY, buffer.readString(Charset.defaultCharset()));
            }else {
                fastLog(id, MessageType.REQUEST_HEADER,
                        CONTENT_TYPE + HEADER_DELIMITER + SPACE + buffer.readString(Charset.defaultCharset()));
